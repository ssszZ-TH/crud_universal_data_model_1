...
switch ($method) {
    case 'GET':
        ...
        if (isset($_GET['id'])) {
            // Handle Read by ID
            $id = $_GET['id'];
            $stmt = $pdo->prepare('SELECT * FROM public.maritalstatustype WHERE maritalstatustypeid = ?');
            $stmt->execute([$id]);
            ...
            // Handle Read all
            $stmt = $pdo->query('SELECT * FROM public.maritalstatustype ORDER BY maritalstatustypeid ASC');
    ...
    case 'POST':
        ...
        if (isset($data['description'])) {
            ...
            $stmt = $pdo->prepare("INSERT INTO public.maritalstatustype(description) VALUES (?)");
    ...
    case 'PUT':
        ...
        if (isset($_GET['id'])) {
            ...
            if (isset($data['description'])) {
                ...
                $stmt = $pdo->prepare('UPDATE public.maritalstatustype SET description = ? WHERE maritalstatustypeid = ?');
                $stmt->execute([$data['description'], $id]);

                // ดึงข้อมูลที่เพิ่งอัปเดต
                $selectStmt = $pdo->prepare('SELECT * FROM public.maritalstatustype WHERE maritalstatustypeid = ?');
    ...
    case 'DELETE':
        ...
        if (isset($_GET['id'])) {
            ...
            if ($dataToDelete) {
                // ลบข้อมูล
                $stmt = $pdo->prepare('DELETE FROM public.maritalstatustype WHERE maritalstatustypeid = ?');
                $stmt->execute([$id]);
