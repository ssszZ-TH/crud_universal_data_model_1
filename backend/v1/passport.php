...
switch ($method) {
    case 'GET':
        ...
        if (isset($_GET['id'])) {
            // Handle Read by ID
            $id = $_GET['id'];
            $stmt = $pdo->prepare('SELECT * FROM public.passport WHERE passportid = ?');
            $stmt->execute([$id]);
            ...
            // Handle Read all
            $stmt = $pdo->query('SELECT * FROM public.passport ORDER BY passportid ASC');
    ...
    case 'POST':
        ...
        if (isset($data['passportnum'], $data['issuedate'], $data['expirationdate'])) {

            // เตรียม statement ตอนเเรกลองเอาประโยค query เก็บในตัวเเปรก่อนค่อยนําไปใช้ จะทำให้ program bug
            $stmt = $pdo->prepare("INSERT INTO public.passport(passportnum, issuedate, expirationdate) VALUES (?, ?, ?)");

            // Execute คำสั่ง SQL พร้อมข้อมูลที่รับมา
            $stmt->execute([$data['passportnum'], $data['issuedate'], $data['expirationdate']]);
    ...
    case 'PUT':
        ...
        if (isset($_GET['id'])) {
            ...
            $selectStmt = $pdo->prepare('SELECT * FROM public.passport WHERE passportid = ?');
            ...
            if ($existingData) {
                ...
                if (isset($data['passportnum'], $data['issuedate'], $data['expirationdate'])) {
                    ...
                    $stmt = $pdo->prepare('UPDATE public.passport SET passportnum = ?, issuedate = ?, expirationdate = ? WHERE passportid = ?');
                    $stmt->execute([$data['passportnum'], $data['issuedate'], $data['expirationdate'], $id]);
    ...
    case 'DELETE':
        ...
        if (isset($_GET['id'])) {
            ...
            if ($dataToDelete) {
                // ลบข้อมูล
                $stmt = $pdo->prepare('DELETE FROM public.passport WHERE passportid = ?');
                $stmt->execute([$id]);
