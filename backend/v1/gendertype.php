...
switch ($method) {
    case 'GET':
        ...
        if (isset($_GET['id'])) {
            // Handle Read by ID
            $id = $_GET['id'];
            $stmt = $pdo->prepare('SELECT * FROM public.gendertype WHERE gendertypeid = ?');
            $stmt->execute([$id]);
            ...
            // Handle Read all
            $stmt = $pdo->query('SELECT * FROM public.gendertype ORDER BY gendertypeid ASC');
    ...
    case 'POST':
        ...
        if (isset($data['gendercode'], $data['description'])) {

            // เตรียม statement ตอนเเรกลองเอาประโยค query เก็บในตัวเเปรก่อนค่อยนําไปใช้ จะทำให้ program bug
            $stmt = $pdo->prepare("INSERT INTO public.gendertype(gendercode,description) VALUES (?, ?)");

            // Execute คำสั่ง SQL พร้อมข้อมูลที่รับมา
            $stmt->execute([$data['gendercode'], $data['description']]);
    ...
    case 'PUT':
        ...
        if (isset($_GET['id'])) {
            ...
            if (isset($data['gendercode'], $data['description'])) {
                ...
                $stmt = $pdo->prepare('UPDATE public.gendertype SET gendercode = ?, description = ? WHERE gendertypeid = ?');
                $stmt->execute([$data['gendercode'], $data['description'], $id]);

                // ดึงข้อมูลที่เพิ่งอัปเดต
                $selectStmt = $pdo->prepare('SELECT * FROM public.gendertype WHERE gendertypeid = ?');
                $selectStmt->execute([$id]);
                $updatedData = $selectStmt->fetch(PDO::FETCH_ASSOC);

                // ส่ง response กลับไปพร้อมข้อมูลที่อัปเดต
                sendResponse(200, $updatedData, 'updated');
            } else {
    ...
    case 'DELETE':
        ...
        if (isset($_GET['id'])) {
            ...
            if ($dataToDelete) {
                // ลบข้อมูล
                $stmt = $pdo->prepare('DELETE FROM public.gendertype WHERE gendertypeid = ?');
                $stmt->execute([$id]);
